# %load /content/drive/MyDrive/cultural-trends/scripts/wvs_compute_alignment.py
#%%writefile /content/drive/MyDrive/cultural-trends/scripts/wvs_compute_alignment.py
import argparse
import os
import re
import scipy
...
# Survey Persona Demographic Columns
demographic_txt = [
    "sex",
    "age",
    "education",
...
#              ]
# Model persona-questions
columns_by = [
              'persona.sex',
              'persona.age',
              'persona.education',
              'persona.employment_status',
              'persona.occupation_group',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prompt-variant', default=1, type=int, help="Prompt variant to use")
    ...
    # Modified MODELS list to align with previous processing output
    MODELS = ['Llama-2-13b-chat-hf']  # Previously: ['AceGPT-13B-chat', 'Llama-2-13b-chat-hf', "gpt-3.5-turbo-1106", "mt0-xxl"]
        ...
        for MODEL in MODELS:
            for LANG in LANGS:
                    ...
                    for qidx in selected_questions:
                        if qidx in skip_questions:
                            continue
                        # if qidx in invalid_questions: continue
                        # q=236_country=us_lang=en_model=gpt-3.5-turbo-0613_eval=mv_sample.csv

                        results_path = f"../dumps_wvs_2/q={qidx}_country={model_country}_lang={LANG}_model={MODEL}_eval={EVAL_METHOD}_pv{args.prompt_variant}.csv"

                        if not os.path.exists(results_path):
                            results_path = results_path.replace(f"_pv{args.prompt_variant}", "")


                        if os.path.exists(results_path):
                            results_df = pd.read_csv(results_path).drop(columns='Unnamed: 0')
                            if "question.prompt_variant" not in results_df.columns:
                                results_df["question.prompt_variant"] = PROMPT_VARIANT
                    ...
                    if question_results:
                        all_results += [pd.concat(question_results, ignore_index=True)] # all questions in single df
                        result_config += [(model_country, LANG, MODEL)]
                    else:
                        print(f"> No results found for config: {(model_country, LANG, MODEL)}")

    # columns_by = ['persona.region', 'persona.sex', 'persona.age', 'persona.country', 'persona.marital_status', 'persona.education', 'persona.social_class', 'question.id', 'question.variant']
    # columns_by = ['persona.region', 'persona.sex', 'persona.age']

    if all_results:
        print(f"Total : {len(all_results[0])}") #persona- model
    else:
        raise RuntimeError("No dump files were loaded. Check dump directories and configuration.")
    ...
    print(f"Number of dataframes {len(all_results)}")
    print(f"Syhthetic Persona Count: {len(personas)}")

    result_json = []
    unique_personas = set()

    for persona_tuple in personas:
        unique_personas.add(persona_tuple[:-3])

    # Filter each result dataframe to only have unique personas
    ...
    # Filter survey df to only have unique personas
    ...
    print(f"Survey DF complete: {len(survey_df)}")
    for remove_idx in remove_indices[::-1]:
        survey_df.drop(remove_idx, inplace=True)

    print(f"Survey DF filter : {len(survey_df)}")
    # breakpoint()
    for qidx in selected_questions:
        if qidx in skip_questions:
            continue
    ...
    for result_idx, result in enumerate(results):
        ...
        for qidx in tqdm(selected_questions):
            ...
            for persona in unique_personas:
                ...
                #persona_row = question_result_df[
                    #(question_result_df["persona.region"] == persona[0])

                    #&(question_result_df["persona.sex"] == persona[1]) &
                    #(question_result_df["persona.age"] == persona[2]) &
                   # (question_result_df["persona.marital_status"] == persona[3]) &
                    #(question_result_df["persona.education"] == persona[4])

                    #(question_result_df["persona.social_class"] == persona[5])
                persona_row = question_result_df[
                    (question_result_df["persona.sex"] == persona[0]) &
                    (question_result_df["persona.age"] == persona[1]) &
                    (question_result_df["persona.education"] == persona[2]) &
                    (question_result_df["persona.employment_status"] == persona[3]) &
                    (question_result_df["persona.occupation_group"] == persona[4]) &
                    (question_result_df["persona.income_level"] == persona[5]) &
                    (question_result_df["persona.religion"] == persona[6]) &
                    (question_result_df["persona.ethnicity"] == persona[7])
                ]
                ...
                survey_persona_row = survey_df[
                    ...
                    (survey_df[demographic_ids[6]] == persona[6]) &
                    (survey_df[demographic_ids[7]] == persona[7])
                ]
                   ...
                   # other_survey_answer = other_survey_persona_row[other_wvs_question_map[qidx]].item()
                    ...
                    for variant_idx, model_answer in enumerate(model_answers):
                        ...
                        persona_json_results += [{
                            "question": qidx,
                           # "variant": variant_idx,
                            #"persona.region": persona[0],
                            #"persona.sex": persona[1],
                            #"persona.age": persona[2],
                            #"persona.marital_status": persona[3],
                            #"persona.education": persona[4],
                            #"persona.social_class": persona[5],
                            
                            "persona.sex": persona[0],
                            "persona.age": persona[1],
                            "persona.education": persona[2],
                            "persona.employment_status": persona[3],
                            "persona.occupation_group": persona[4],
                            "persona.income_level": persona[5],
                            "persona.religion": persona[6],
                            "persona.ethnicity": persona[7],
                            ...
                            "rand-mae": persona_random[-1],

                        }]

                except Exception as e:
                    print(f"Skipping persona {persona} for Q{qidx} because of error: {e}")
                    continue
    ...
    if SKIP_SAME_ANS:
        ...
        df = pd.read_json(f"persona_results_{SURVEY_COUNTRY}_filtered_all_3.json")
        df.to_csv(f"persona_results_{SURVEY_COUNTRY}_filtered_all_3.csv", index=False)
        

        # print(f"{result_config[result_idx]}: {score} | {np.mean(final_exact)} | {len(persona_exact)}")

print("Final rows:", len(persona_json_results))
